    ...
    _accMarginXY(30.*cm)
{
  ...
  _messenger->DeclarePropertyWithUnit("accMarginXY", "mm", _accMarginXY,
    "Additive margin added to the true plate half-extent when checking acceptance (mm).");
...
if (_detType == "triangular") {
    ...
    *_rotLowerY *= *_rotLowerX;
    ...
    auto maxZIt = std::max_element(std::begin(_zPosStations), std::end(_zPosStations));
    auto minZIt = std::min_element(std::begin(_zPosStations), std::end(_zPosStations));
    G4int frontIdx = std::distance(std::begin(_zPosStations), maxZIt);
    G4int backIdx  = std::distance(std::begin(_zPosStations), minZIt);

    _plateZMax = _zPosStations[frontIdx] + 0.5*layerThickness;
    _plateZMin = _zPosStations[backIdx]  - 0.5*layerThickness;

    _frontCenterY = _yPosStations[frontIdx]; // entry-plate vertical center
    _backCenterY  = _yPosStations[backIdx];  // exit-plate vertical center

    // Preserve the TRUE bar-layer transverse footprint before _halfContLengthXY
    // gets redefined below for unrelated container-sizing purposes.
    _plateHalfXY = (std::max(_barLength, (_barBase + xySafety) * (_nBars + 0.5))) / 2.;
...
_worldOffsetX = 0.5*_AlShellHeight+0.5*layerThickness -(_zPosStations[1]+_AlShellHeight)-0.5*layerThickness;
...
new G4PVPlacement(0,
    ...
    new G4PVPlacement(rotDet,                       //no rotation
      G4ThreeVector(_worldOffsetX,0,0),  // centered in middle of station 1
...
// pos/dir are expected in WORLD-frame coordinates (as generated by CRY/EcoMug primaries).
// detContainerLog's local frame (where _halfContLengthZ/_halfContLengthXY are defined:
// local Z = viewing/depth axis, local X/Y = front-face transverse axes) is related to
// world coordinates by the permutation baked into the rotDet placement of "DetectorMother":
//   world X = local Z + _worldOffsetX   (depth axis, now horizontal)
//   world Y = local X                   (transverse)
//   world Z = local Y                   (vertical "up")
G4bool DetectorConstruction::IsInsideAcceptance(const G4ThreeVector &pos, const G4ThreeVector &dir) const
{
  G4double localPosZ = pos[0] - _worldOffsetX;
  G4double localPosX = pos[1];
  G4double localPosY = pos[2];

  G4double localDirZ = dir[0];
  G4double localDirX = dir[1];
  G4double localDirY = dir[2];

  if (localDirZ == 0.) return false;

  // 1. Reject particles not entering from the front side
  if (localPosZ < _plateZMax)
    return false;

  // 2. Front plate hit test (X centered at 0, Y centered at _frontCenterY)
  G4double hitPoint = fabs(localPosX + (localDirX / localDirZ) * (_plateZMax - localPosZ));
  if (hitPoint > _plateHalfXY + _accMarginXY)
    return false;
  hitPoint = fabs((localPosY + (localDirY / localDirZ) * (_plateZMax - localPosZ)) - _frontCenterY);
  if (hitPoint > _plateHalfXY + _accMarginXY)
    return false;

  // 3. Back plate hit test (X centered at 0, Y centered at _backCenterY)
  hitPoint = fabs(localPosX + (localDirX / localDirZ) * (_plateZMin - localPosZ));
  if (hitPoint > _plateHalfXY + _accMarginXY)
    return false;
  hitPoint = fabs((localPosY + (localDirY / localDirZ) * (_plateZMin - localPosZ)) - _backCenterY);
  if (hitPoint > _plateHalfXY + _accMarginXY)
    return false;
